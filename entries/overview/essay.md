...
*Unicode history, compatibility, and production-boundary guidance*
...
- implementers and standards readers need a clear production boundary

> the official Unicode SignWriting block has Unicode-scoped value as a character repertoire, but it is not a production-preserving Sutton SignWriting text model, a compatible production replacement for the established Sutton SignWriting production ecosystem, or a complete written-sign model
...
- the current official Unicode SignWriting block and model should not be treated as a production-preserving text model or complete production replacement for FSW/SWU
...
- future standards discussion is only worth the effort if it begins from those realities
...
- the difference between naming characters, preserving production-level symbol identity, and encoding complete written signs

## Technical Position

The technical position is:

- the official Unicode SignWriting block has Unicode-scoped value as a character repertoire, but it is not currently a production-preserving text model or compatible production replacement for the established FSW/SWU Sutton SignWriting production ecosystem
...
| Official Unicode SignWriting | Unicode-side SignWriting character repertoire and Unicode-specific composition model introduced in Unicode 8.0.0 | Not a production-preserving text model, compatible production replacement for FSW/SWU, or complete written-sign model |
| Noto Sans SignWriting | Public font path for the official Unicode model | Not proof of audited production readiness, lossless FSW/SWU migration, or independent implementation compatibility |
...
4. **Facial Orthography Boundary**
   End with the practitioner-facing boundary note for official Unicode facial composition and Sutton SignWriting production text.
